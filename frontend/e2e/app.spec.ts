...
test('logs in, streams chat, toggles search mode, and uploads to a knowledge base', async ({
  ...
  await expect(page.getByText(/银行业报告已生成/)).toBeVisible()
  await expect(page.getByText('Agent Timeline')).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '执行摘要' }).first()).toBeVisible()
